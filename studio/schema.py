...
class TrainingConfig(BaseModel):
    ...
    optimizer_type: Literal["adamw", "pion", "prodigy", "prodigy_plus_schedulefree"] = Field(
        "adamw",
        description="优化器（Pion 实验；测试建议 lr=1e-4 + lr_scheduler=none，先保持高级项默认）",
        json_schema_extra=_meta("training"),
    )
    # ---------------- Pion 专属字段 ----------------
    ...
    pion_rms_scale: float = Field(
        1.0, gt=0.0,
        description="Pion 更新 RMS 缩放（默认 1.0；发散降到 0.5，收敛慢可试 1.5-2.0）",
        json_schema_extra=_meta("training", show_when="optimizer_type==pion", advanced=True),
    )
    pion_max_side: int = Field(
        256, ge=1,
        description="Pion 正交旋转最大边长（默认 256；想覆盖更多矩阵可试 512，会更慢/更占显存）",
        json_schema_extra=_meta("training", show_when="optimizer_type==pion", advanced=True),
    )
    pion_beta1: float = Field(
        0.9, ge=0.0, lt=1.0,
        description="Pion 李代数动量 β1（推荐保持 0.9，类似 AdamW 一阶动量）",
        json_schema_extra=_meta("training", show_when="optimizer_type==pion", advanced=True),
    )
    pion_beta2: float = Field(
        0.99, ge=0.0, lt=1.0,
        description="Pion 二阶动量 β2（推荐保持 0.99，小数据 LoRA 比 0.999 更灵敏）",
        json_schema_extra=_meta("training", show_when="optimizer_type==pion", advanced=True),
    )
    pion_use_second_moment: bool = Field(
        True,
        description="Pion 使用二阶动量预条件（推荐开启，步长更稳）",
        json_schema_extra=_meta("training", show_when="optimizer_type==pion", advanced=True),
    )
    pion_alternating: bool = Field(
        True,
        description="Pion 交替更新输入/输出正交侧（推荐开启，速度更友好）",
        json_schema_extra=_meta("training", show_when="optimizer_type==pion", advanced=True),
    )
    pion_fallback_zero: bool = Field(
        True,
        description="Pion 零初始化矩阵回退 AdamW（LoRA/LoKr 推荐开启，否则零矩阵可能学不动）",
        json_schema_extra=_meta("training", show_when="optimizer_type==pion", advanced=True),
    )
    pion_exp: Literal["exact", "second_order"] = Field(
        "exact",
        description="Pion 正交映射（exact 更稳；second_order 更快但近似）",
        json_schema_extra=_meta("training", show_when="optimizer_type==pion", advanced=True),
    )
